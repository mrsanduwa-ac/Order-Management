// --- CONFIGURATION ---
const WEB_APP_URL = "https://script.google.com/macros/s/AKfycby2JfJzXO2T3zWxhael5bSwzDIcqDHL0Qs4-dnzOjbOuIQL7EMZXpN1i3lWdeNeluSp/exec";
...
function attachEventListeners() {
    ...
    el.orderScanInput.addEventListener("keypress", orderInputKeypressHandler);
    ...
    el.scannedOrderBarcodesList.addEventListener('click', scannedListClickHandler);
    ...
    el.passcodeInput.addEventListener("keypress", passcodeInputKeypressHandler);
}

function removeEventListeners() {
    el.orderScanInput.removeEventListener("keypress", orderInputKeypressHandler);
    el.orderScanInput.removeEventListener("input", handleAutoScan);
    el.addOrderBarcodeBtn.removeEventListener("click", handleOrderScan);
    el.savedSessionsSelect.removeEventListener("change", loadSelectedSession);
    el.deleteSelectedSessionBtn.removeEventListener("click", deleteSelectedSession);
    el.searchScannedInput.removeEventListener("input", renderPermanentBarcodes);
    el.clearCurrentOrderBtn.removeEventListener("click", clearCurrentOrder);
    el.scannedOrderBarcodesList.removeEventListener('click', scannedListClickHandler);
    el.showDeletedBtn.removeEventListener('click', showDeletedBarcodes);
    el.printScannedOrderBtn.removeEventListener("click", printScannedOrder);
    el.downloadCsvBtn.removeEventListener("click", downloadCsv);
    el.searchByDate.removeEventListener("input", filterSessionsByDate);
    el.resetDateFilterBtn.removeEventListener("click", () => {
        el.searchByDate.value = "";
        filterSessionsByDate();
    });
    el.passcodeSubmitBtn.removeEventListener("click", handlePasscodeSubmit);
    el.passcodeInput.removeEventListener("keypress", passcodeInputKeypressHandler);
}

function orderInputKeypressHandler(e) {
    if (e.key === "Enter") handleOrderScan();
}

function passcodeInputKeypressHandler(e) {
    if (e.key === "Enter") handlePasscodeSubmit();
}

function scannedListClickHandler(e) {
...
function handleOrderScan() {
    ...
    } else if (deletedBarcodes.has(barcode)) {
        el.errorSound.play();
        showStatusMessage(`Barcode in removed list: ${barcode}`, "error");
    } else {
...
function loadSelectedSession() {
    ...
    if (session) {
        ...
        scannedUniqueBarcodes = new Set((session.data && session.data.uniqueBarcodes) ? session.data.uniqueBarcodes : []);
        deletedBarcodes.clear();
        renderPermanentBarcodes();
        Swal.fire({ title: "Loaded!", text: `Session '${session.name}' has been loaded.`, icon: "success", timer: 1500, showConfirmButton: false });
    }
}
...
function showDeletedBarcodes() {
    ...
    deletedBarcodes.forEach(barcode => {
        deletedItemsHtml += `<li class="flex justify-between items-center p-2 bg-gray-700/50 rounded-md"><span>${barcode}</span> <button onclick="window.restoreBarcode && window.restoreBarcode('${barcode}')" class="btn-success px-2 py-1 rounded-md text-xs font-semibold">Restore</button></li>`;
    });
    deletedItemsHtml += '</ul>';
    Swal.fire({
        ...
        willClose: () => {
            window.restoreBarcode = undefined;
        }
    });
}
...
async function clearCurrentOrder() {
    ...
    const result = await Swal.fire({
        title: "Are you sure?",
        text: "This will clear all current and removed items.",
        icon: "warning",
    ...
    if (result.isConfirmed) {
        ...
        renderPermanentBarcodes();
        Swal.fire({ title: "Cleared!", icon: "info", timer: 1500, showConfirmButton: false });
    }
}
...
async function deleteSelectedSession() {
    ...
    if (result.isConfirmed) {
        try {
            ...
            Swal.fire({ title: "Deleted!", text: "The session has been deleted.", icon: "success", timer: 1500, showConfirmButton: false });
...
function updateClock() {
    el.liveClock.textContent = new Date().toLocaleString("en-US", {
        weekday: "long",
        year: "numeric",
        month: "long",
        day: "numeric",
...
function getTimestamp(epoch) {
    ...
    if (isNaN(epochInt)) return "";
    return new Date(epochInt).toLocaleString("en-US", { dateStyle: "short", timeStyle: "short", hour12: false });
}
